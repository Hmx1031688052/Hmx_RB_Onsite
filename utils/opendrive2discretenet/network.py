        ...
        for junction in opendrive.junctions:
            for connection in junction.connections:
                ...
                if incoming_road is None or connecting_road is None:
                    warnings.warn(
                        "Skipping OpenDRIVE junction connection with missing road: "
                        f"junction={junction.id}, connection={connection.id}, "
                        f"incomingRoad={connection.incomingRoad}, "
                        f"connectingRoad={connection.connectingRoad}"
                    )
                    continue

                for lane_link in connection.laneLinks:
                    if lane_link.fromId == 0 or lane_link.toId == 0:
                        warnings.warn(
                            "Skipping OpenDRIVE junction laneLink through center lane: "
                            f"junction={junction.id}, connection={connection.id}, "
                            f"from={lane_link.fromId}, to={lane_link.toId}"
                        )
                        continue
